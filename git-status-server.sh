...
ssh -p $PORT $SERVER "cd $REMOTE_DIR && git branch --show-current 2>/dev/null || echo 'N/A'"

echo ""

# Mostrar último commit
echo -e "${YELLOW}📝 Último commit:${NC}"
ssh -p $PORT $SERVER "cd $REMOTE_DIR && git log -1 --oneline 2>/dev/null || echo 'Sin commits'"

echo ""

# Mostrar estado
echo -e "${YELLOW}📊 Estado del repositorio:${NC}"
ssh -p $PORT $SERVER "cd $REMOTE_DIR && git status -s 2>/dev/null || echo 'Git no disponible'"
